#!/usr/bin/env python3
"""Switch wRC+ to event-level linear weights. Run from project root."""
APP = "src/App.jsx"
src = open(APP).read()
changes = 0

# 1. Statcast engine
old_wrc = "  const wrc = Math.max(60, Math.min(195, Math.round((ops / 0.720) * 100 + db)));"
new_wrc = """  // wRC+ from event-level linear weights (2024 FanGraphs)
...
if old_wrc in src:
    ...
    changes += 1
    print("1. Statcast wRC+: event-level linear weights")

# 2. Marcel engine - replace BOTH duplicate lines at once
old_marcel = """  // Recalculate wRC+ from final displayed OPS (ensures correlation)
...
new_marcel = """  // wRC+ from event-level linear weights (2024 FanGraphs)
...
if old_marcel in src:
    ...
    changes += 1
    print("2. Marcel wRC+: event-level linear weights (fixed duplicate)")
