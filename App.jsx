...
let WAR_DATA = {};
try { WAR_DATA = await import("./war_data.json"); } catch {}
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12}}>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {base&&<>
              ...
              {(()=>{const cw=getCareerWAR(player.id);return cw!==null?<Stat label="Career fWAR" value={cw.toFixed(1)} color={cw>=30?C.green:cw>=15?C.blue:C.purple} sub="FanGraphs"/>:null;})()}
...
const LEADER_COLS = [
  ...
  { k: "careerWAR",l: "Career WAR",   fmt: v => v?.toFixed(1) ?? "—", align: "right", sortDir: -1 },
  { k: "tenYrWAR", l: "10yr WAR",     fmt: v => v?.toFixed(1) ?? "—", align: "right", sortDir: -1 },
    ...
    for (let i = 0; i < hitters.length; i += BATCH) {
      ...
      const batchResults = await Promise.all(batch.map(async (p) => {
        try {
          ...
          const tenYrWAR = forward.reduce((s, d) => s + Math.max(0, d.war), 0);
          const fv = getPlayerFV(p.id, p.fullName);

          // Career WAR: use FanGraphs data if available, otherwise estimate from stats
          const fgWAR = getCareerWAR(p.id);
          let careerWAR;
          if (fgWAR !== null) {
            careerWAR = fgWAR;
          } else {
            // Estimate from career MLB stats
            const ap = getAP(p.primaryPosition?.code);
            careerWAR = 0;
            splits.filter(s => s.stat?.plateAppearances > 50).forEach(s => {
              const st = s.stat;
              const lvl = detectLevel(s);
              if (lvl !== "MLB") return;
              const pa = st.plateAppearances || 0;
              const ops = parseFloat(st.ops || 0);
              const wrc = Math.round((ops / 0.720) * 100);
              const bat = ((wrc - 100) / 100) * pa * 0.12;
              const pos = ap.pa * (pa / 600);
              const rep = 20 * (pa / 600);
              careerWAR += (bat + pos + rep) / 10;
            });
          ...
          return {
            ...
            careerWAR: careerWAR,
            tenYrWAR: Math.round(tenYrWAR * 10) / 10,
