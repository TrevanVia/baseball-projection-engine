...
function projectFromStatcast(sP, age, posCode, playerName, playerId) {
  ...
  const bat = battingRunsFromWOBA(projWOBA, ePA);
...
function projectFromSeasons(splits, age, posCode, playerName, playerId) {
  ...
  // Batting runs from wOBA (bridge from OPS for first-pass WAR calc)
  const marcelWOBA = Math.max(0.200, Math.min(0.480, finalOPS * 0.430));
  const batRuns = battingRunsFromWOBA(marcelWOBA, estPA);
...
function PlayerCard({player}) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      {!isPitcher && seasons.length > 0 && (
        <Panel title="OPS TRAJECTORY" sub="Historical season OPS by level + 3 projected seasons. Dashed line = league average (.720).">
          <ResponsiveContainer width="100%" height={280}>
            <ComposedChart data={[
              ...seasons.map(s => ({ season: s.season, ops: s.ops, level: s.level, type: "actual" })),
              ...opsTrajectory.map(s => ({ ...s, type: "projected" })),
            ]} margin={{top:10,right:20,left:0,bottom:0}}>
              <CartesianGrid strokeDasharray="3 3" stroke={C.grid}/>
              <XAxis dataKey="season" stroke={C.muted} fontSize={10} fontFamily={F}/>
              <YAxis stroke={C.muted} fontSize={10} fontFamily={F} domain={["auto","auto"]}/>
              <Tooltip content={<Tip/>}/>
              <ReferenceLine y={0.720} stroke={C.muted} strokeDasharray="5 5" label={{value:"Lg Avg",position:"right",fontSize:9,fill:C.muted,fontFamily:F}}/>
              <Bar dataKey="ops" name="OPS" radius={[3,3,0,0]} barSize={22}>
                {[
                  ...seasons.map(s => ({ level: s.level, type: "actual" })),
                  ...opsTrajectory.map(s => ({ level: "PROJ", type: "projected" })),
                ].map((d, i) => (
                  <Cell key={i} fill={d.type === "projected" ? C.accent : (LEVEL_COLORS[d.level] || C.blue)} fillOpacity={d.type === "projected" ? 0.55 : 0.75}/>
                ))}
              </Bar>
            </ComposedChart>
          </ResponsiveContainer>
