...
# 3. Fix remaining `af` references - HR calc used af which is now gone
old_hr = """const hr=Math.round(Math.max(0,pBrl/100*(ePA*.75)*.24*af));"""
new_hr = """const hr=Math.round(Math.max(0,pBrl/100*(ePA*.75)*.24));"""
if old_hr in src:
    src = src.replace(old_hr, new_hr)
    changes += 1
    print("2. Fixed HR calculation (removed undefined af reference)")
