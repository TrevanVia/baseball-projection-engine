...
function PlayerCard({player}) {
  ...
  useEffect(()=>{setLoading(true);getPlayerCareer(player.id).then(s=>{setCareer(s);setLoading(false);});},[player.id]);
  ...
  }).sort((a,b)=>parseInt(a.season)-parseInt(b.season)),[career,player]);

  const base = useMemo(()=>career.length?projectFromSeasons(career,player.currentAge,player.primaryPosition?.code):null,[career,player]);
